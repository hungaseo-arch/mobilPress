...
async function submit() {
  ...
  try {
    ...
    toast.error(error instanceof Error ? error.message : '비밀번호 재설정에 실패했습니다.')
  } finally {
...
<template>
  <div class="flex min-h-screen items-center justify-center bg-slate-50 px-4">
    <div class="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
      <h1 class="text-xl font-bold text-slate-900">비밀번호 재설정</h1>
      ...
      <div v-if="linkError || !token" class="mt-4 space-y-4">
        ...
        <p class="text-sm text-slate-500">
          로그인 화면에서 "비밀번호를 잊으셨나요?" 를 눌러 새 링크를 요청해주세요.
        </p>
        <RouterLink
          to="/"
