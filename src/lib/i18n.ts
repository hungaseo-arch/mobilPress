...
const messages: Record<string, { id: string; ko: string }> = {
  ...
  // 로그인 / 인증
  'auth.subtitle.signin': { id: 'Masuk dengan akun tim Anda.', ko: '팀 계정으로 로그인하세요.' },
  'auth.subtitle.signup': { id: 'Buat akun baru.', ko: '새 계정을 만듭니다.' },
  'auth.subtitle.forgot': {
    id: 'Kami akan mengirim tautan reset ke email terdaftar.',
    ko: '가입한 이메일로 재설정 링크를 보내드립니다.',
  },
  'auth.name': { id: 'Nama', ko: '이름' },
  'auth.email': { id: 'Email', ko: '이메일' },
  'auth.password': { id: 'Kata sandi (min. 8 karakter)', ko: '비밀번호 (8자 이상)' },
  'auth.signin': { id: 'Masuk', ko: '로그인' },
  'auth.signup': { id: 'Daftar', ko: '회원가입' },
  'auth.sendReset': { id: 'Kirim tautan reset', ko: '재설정 링크 발송' },
  'auth.google': { id: 'Lanjutkan dengan Google', ko: 'Google 로 계속하기' },
  'auth.backToSignin': { id: 'Kembali ke halaman masuk', ko: '로그인으로 돌아가기' },
  'auth.toSignup': { id: 'Belum punya akun? Daftar', ko: '계정이 없나요? 회원가입' },
  'auth.forgot': { id: 'Lupa kata sandi?', ko: '비밀번호를 잊으셨나요?' },
  'auth.resetSent': {
    id: 'Tautan reset telah dikirim ke email Anda. Periksa kotak masuk.',
    ko: '재설정 링크를 이메일로 보냈습니다. 메일함을 확인하세요.',
  },
  ...
  'reset.invalid': { id: 'Tautan reset tidak valid atau sudah kedaluwarsa.', ko: '재설정 링크가 유효하지 않거나 만료되었습니다.' },
  'reset.invalidHint': {
    id: 'Klik "Lupa kata sandi?" di halaman masuk untuk meminta tautan baru.',
    ko: '로그인 화면에서 "비밀번호를 잊으셨나요?" 를 눌러 새 링크를 요청해주세요.',
  },
  'reset.toSignin': { id: 'Ke halaman masuk', ko: '로그인 화면으로' },
  'reset.newPassword': { id: 'Kata sandi baru (min. 8 karakter)', ko: '새 비밀번호 (8자 이상)' },
  'reset.confirmPassword': { id: 'Konfirmasi kata sandi baru', ko: '새 비밀번호 확인' },
  'reset.submit': { id: 'Ubah kata sandi', ko: '비밀번호 변경' },
