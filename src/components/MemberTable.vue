    ...
    <div class="mb-5 flex flex-wrap items-center justify-between gap-2">
      <div class="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
        <span
          ...
          :key="r"
          class="rounded-full border border-border bg-card px-3 py-1.5 font-medium"
        >
          ...
          <tbody>
            ...
            >
              <td class="px-4 py-3 text-foreground">
                {{ member.email }}
                <span v-if="isSelf(member)" class="ml-1.5 rounded-full bg-primary-soft px-2 py-0.5 text-xs text-primary">
                  {{ t('member.self') }}
                </span>
              </td>
              ...
              <td class="px-4 py-3">
                <div class="flex items-center gap-2">
                  <select
                    ...
                    class="rounded-md border border-border bg-secondary px-2.5 py-1.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-60"
