...
2. 이 저장소의 [`db/schema.sql`](../db/schema.sql) 내용 전체를 붙여넣고 **Run** (여러 번 실행해도 안전)
   ...
   - 권한 정책 (역할 3단계):
     ...
     | staff | ✅ | ✅ | ❌ |
     | user (기본값) | ✅ | ❌ | ❌ |
     - 승격은 SQL Editor 에서 (`<USER_ID>` 는 콘솔 Auth → Users 에서 복사):
       ```sql
...
| 데이터 권한 | Postgres RLS | 조회: 로그인 전원 / 입력·수정: staff 이상 / 삭제: admin, 익명 차단 |
| 역할 관리 | `public.user_roles` | admin(전체) / staff(입력·수정) / user(조회, 기본값). SQL Editor 로 승격 |
