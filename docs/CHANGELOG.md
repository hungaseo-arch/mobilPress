...
## 2026-09-10 — 조회 전용(user) 계정 마스킹: 금액·고객명·위치

조회 전용 아이디로 접속하면 **금액·고객명·위치**가 가려져 보입니다. 외부에 화면을 보여줘야 하는
상황에서 거래 규모와 거래처가 그대로 드러나던 문제를 막기 위한 것입니다. **SQL 마이그레이션 없음.**

- 정책은 `auth-state.ts` 한 곳에서 정합니다 — `maskSensitive = authEnabled && role === 'user'`.
  mock/proxy 모드(로그인 게이트가 없는 로컬 개발)에서는 마스킹하지 않습니다
- 표시 helper 는 `src/lib/mask.ts` 로 모았습니다. 마스킹 대상이 아니면 기존 서식 함수와 똑같이
  동작하므로 호출부는 `formatIDR(x)` → `maskedIDR(x)` 처럼 바꾸기만 하면 됩니다
  - 금액 → `Rp ••••••` **고정 길이** (자릿수로 금액 규모가 드러나지 않도록). 값이 0 일 때 `-` 로
    두던 자리도 가려서, 값의 유무조차 노출하지 않습니다
  - 고객명·요청고객·지역·담당자명 → **단어 첫 글자만** (`PT. Astra Otoparts` → `P•• A•••• O•••••••`).
    전부 점으로 덮으면 모든 행이 똑같아져 표·순위를 읽을 수 없으므로 행 구분은 남깁니다
  - 할인율·시장가격·좌표/지도 링크 → `••••` (할인율은 서비스 비용과 조합하면 단가가 역산됩니다)
  - 첨부 파일명 → `LK_20260814_••••_2PCS.pdf` (표준 파일명에 고객명이 들어갑니다)
- **화면 밖 우회 경로도 함께 막았습니다.** 마스킹은 표 셀만 가려서는 의미가 없습니다.
  - 작업보고서 스캔(PDF) **미리보기·다운로드 차단**(`REPORT_POLICY.user`) — 스캔 원본에 고객명·위치·
    금액이 그대로 있어 열람 한 번으로 마스킹이 무의미해집니다
  - 검색에서 고객명·요청고객 **제외**(`filteredInstallations`) — 가려진 이름을 입력해 결과 유무로
    되짚어 확인하는 것을 막습니다. 안내 문구도 `search.placeholderMasked` 로 바뀝니다
  - 장착 상세 모달의 고객명 자동완성(`<datalist>`)과 구글 지도 링크 suppress
- 운영자료 탭: 열 이름으로 판별해 회사명·경쟁사 Press 업체·금액·시장 가격 열을 가립니다.
  **⑤ 서비스 요금 탭은 예외** — 회사 표준 단가표라 특정 고객의 거래 금액이 아니고, 표 밖 문구
  ("최소 청구: Rp 500,000")는 가려지지 않아 같은 화면에서 표만 `••••` 로 나오면 오히려 어긋나 보입니다
- `docs/ROLES.md` 의 역할 표에 마스킹·보고서 열람 항목을 추가했습니다

> ⚠️ **한계**: 이것은 화면 표시 마스킹입니다. Data API 응답에는 원값이 그대로 들어오므로 개발자도구로는
...
Auth → Users 에서만 가능했습니다.

- ⚠️ **SQL 마이그레이션 필요**: `sql/2026-09-07_member_delete.sql` 을 Neon SQL Editor 에서 1회 실행한 뒤
  Data API → **Refresh schema cache** (`POST /rpc/delete_member` 노출). `db/schema.sql` 에도 반영
- `public.delete_member(target_user_id text)` — security definer 함수. 계정 원본(`neon_auth."user"`)은
  Data API 가 노출하지 않는 스키마에 있어 직접 지울 수 없으므로, `public` 에 함수를 두고 그 안에서
  `is_admin()` 과 본인 여부(`target_user_id <> auth.user_id()`)를 서버에서 재검증합니다. 실행 권한은
  `authenticated` 에만 부여
- 삭제 범위: `user_roles` 행 + Better Auth 의 `session` / `account`(즉시 로그아웃·재로그인 불가) + `user` 행.
  **접속 기록(`access_logs`)과 감사 로그(`audit_logs`)는 남깁니다** — 계정이 사라져도 "누가 언제 무엇을"은
  보존되어야 하므로. 삭제 사실도 `audit_logs` 에 `table_name='user_accounts'` 로 이메일·이름 스냅샷과 함께 기록
- **본인 계정은 삭제 불가** — 화면에서 버튼을 숨기고 함수에서도 거부합니다(마지막 admin 이 스스로 사라지는
  사고 방지, 역할 변경과 같은 규칙). 그런 경우의 수단은 Neon 콘솔 Auth → Users
- 화면: `MemberTable` 에 '작업' 열과 휴지통 버튼(확인 창 1회, 진행 중 스피너) 추가.
  `neon-api` / `mock-api` 는 `DELETE /mobil-press/members/:userId` 로 각각 RPC 호출과 더미 배열 삭제를 처리
