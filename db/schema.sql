...
create table if not exists public.installations (
  ...
  install_area     text not null default '',   -- 장착 지역 (작업지시서 Lokasi)
  location_url     text not null default '',   -- 구글 위치 (lat,lng 또는 Maps 링크)
...
-- 장착 위치 컬럼 추가 (재실행 안전, sql/2026-09-10_installation_location.sql 과 동일)
alter table public.installations add column if not exists install_area text not null default '';
alter table public.installations add column if not exists location_url text not null default '';
...
-- ─────────────────────────────────────────────
-- 6. 계정 삭제 (회원관리 탭, admin 전용)
--    계정 원본은 Neon Auth 가 관리하는 neon_auth."user" 에 있고 Data API 는 public
--    스키마만 노출하므로, security definer 함수 안에서 권한을 재검증한 뒤 지운다.
--    본인 계정은 막는다 — 마지막 admin 이 스스로 사라지는 사고 방지(역할 변경과 동일).
--    접속 기록·감사 로그는 남긴다 — 계정이 사라져도 이력은 보존되어야 하므로.
-- ─────────────────────────────────────────────
